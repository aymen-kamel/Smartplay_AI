...
def main():
    if not os.path.exists("advanced_shot_features.csv"):
        print("Run collect_advanced_shot_data.py first!")
        return

    df = pd.read_csv("advanced_shot_features.csv")
    ...
    try:
        ...
        df.to_csv("advanced_shot_features_labeled.csv", index=False)
        print("\nProgress saved to advanced_shot_features_labeled.csv")
        cap.release()
